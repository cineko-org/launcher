...
export const Login: Story = { render: (args) => <InteractiveLogin {...args} /> };

function InteractiveLogin(args: ComponentProps<typeof LauncherView>) {
  const [pin, setPin] = useState('');
  return <LauncherView {...args} pin={pin} onPinChange={setPin} />;
}
...
export const MobileLogin: Story = {
  ...
  render: (args) => <InteractiveLogin {...args} />,
};
export const MobileError: Story = {
  globals: { viewport: { value: 'phone', isRotated: false } },
  args: { state: { mode: 'error', message: '서버 응답이 없습니다. 잠시 후 다시 시도하세요.', version: '1.0.0' } },
};
export const MobileUpdate: Story = {
  globals: { viewport: { value: 'phone', isRotated: false } },
  args: { state: { mode: 'updating', stage: 'downloading', message: '업데이트 다운로드 중', artifact: 'client', downloaded: 38, total: 100, version: '1.0.0' } },
};
